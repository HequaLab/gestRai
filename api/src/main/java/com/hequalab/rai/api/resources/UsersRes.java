...
public class UsersRes extends AbstractRes {
	...
	public Page<UserView> list(@Auth UserView user,
		...
		List<UserView> dnv = hibSess().createQuery(
	...
	@Timed
	@CacheControl(noCache = true)
	public UserView fetch(@Auth UserView user, @PathParam("id") UserIdParam id) {
	...
	@UnitOfWork
	@Timed
	public UserView update(@Auth UserView user,
			@PathParam("id") UserIdParam id, @Valid UserUpdate rep)
		...
		String divisione = rep.getDivisione() != null ? rep.getDivisione()
				: recOld.getDivisione();
		String email = rep.getEmail() != null ? rep.getEmail() : recOld
				.getEmail();
		String superiore = rep.getSuperiore() != null ? rep.getSuperiore() : recOld.getSuperiore();
		Set<Role> roleOld = recOld.getRoles();
		
		
		aggSess().save(
				aggSess().get(User.class, id.get()).update(userName, firstName,
						lastName, email, divisione,superiore));

		if ((rep.getRoles() != null && (user.getRoles()
				.contains(Role.Consorzio) || user.getRoles().contains(
				Role.Manager)))) {
			...
			aggSess()
					.save(aggSess().get(User.class, id.get()).addRole(
							rep.getRoles()));
		}
	...
	public void updatePassword(@Auth UserView user,
			...
			throws IllegalAccessException {
		aggSess().save(
				aggSess().get(User.class, id.get()).updatePassword(
						rep.getPassword()));
	...
	@UnitOfWork
	@Timed
	public void deleteRole(@Auth UserView user,
			@PathParam("id") UserIdParam id, @PathParam("role") Role role)
			throws IllegalAccessException {
