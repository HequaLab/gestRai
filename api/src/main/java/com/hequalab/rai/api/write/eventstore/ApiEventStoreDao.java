...
public class ApiEventStoreDao implements EventStoreDao<ApiContext> {
	...
	public synchronized EventStream<ApiContext> append(Identity id,
		...
		try {
			serializedEvents = mapper.writerWithType(
		...
		long createdOnInMillis = createdOn.toDateTime(DateTimeZone.UTC)
				.getMillis();
		String createdOnToString = createdOn.toString(DateTimeFormat
				.forPattern("yyyy-MM-dd hh:mm:ss.SSS"));
		session().save(
				new ApiEventStoreEntry(id.toString(), createdOnToString,
						createdOnInMillis, version, context.getCurrent()
								.userId(), serializedEvents));
	...
	public Iterable<EventStream<ApiContext>> get(Identity id, long version) {
		...
		List<ApiEventStoreEntry> entries = criteria(id.toString()).add(
				Restrictions.le("version", version)).list();
